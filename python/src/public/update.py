...
def update_hunger_value(hunger_list, Popsize, fit, min, max, g_best, g_worst, eps, LH):
    for i in range(Popsize):
        ...
        H = (0.5 * (fit[i, 0] - g_best[i, 0]) + 0.5 * (fit[i, 1] - g_best[i, 1])) / (
                    0.5 * (g_worst[0] - g_best[i, 0]) + 0.5 * (g_worst[1] - g_best[i, 1]) + eps) * r * 2 * space
        if H < LH:
            H = LH * (1 + r)
            hunger_list[i] += H
...
def update_ppso_v(particles, v_,v_min,v_max,in_,in_pbest,in_gbest, delta_list):
    aa = np.zeros(delta_list.shape[0])
    bb = np.zeros(delta_list.shape[0])
    ee = np.zeros(delta_list.shape[0])
    tt = np.zeros(delta_list.shape[0])
    for i in range(particles):
        aa[i] = 2 * (np.sin(delta_list[i]))
        bb[i] = 2 * (np.cos(delta_list[i]))
        ee[i] = abs(np.cos(delta_list[i])) ** aa[i]
        tt[i] = abs(np.sin(delta_list[i])) ** bb[i]
    ...
    for i in range(particles):
        v_[i] = ee[i] * (in_pbest[i] - in_[i]) + tt[i] * (in_gbest[i] - in_[i])
        v_[i] = ensure_min_max_constraints(v_[i])
        v_[i] = adjust_slack_power(v_[i])  

        delta_list[i] += abs(aa[i] + bb[i]) * (2 * np.pi)

    return v_, delta_list
...
def operatorGAhalf(archive_in, proC, disC, proM, disM, lower, upper):

    # Simulated binary crossover
    if (archive_in.shape[0] & 1) == 0:  
        parent1 = archive_in[:(len(archive_in)//2)]
        parent2 = archive_in[(len(archive_in) // 2):]
    else:  
        parent1 = archive_in[:(len(archive_in)//2)]
        parent2 = archive_in[(len(archive_in) // 2) + 1:]
    N, D = parent1.shape
    beta = np.zeros([N, D])
    mu = np.random.rand(N, D)
    beta[mu <= 0.5] = (2 * mu[mu <= 0.5]) * (1 / (disC + 1))
    beta[mu > 0.5] = (2 - 2 * mu[mu > 0.5]) * (-1 / (disC + 1))
    beta = beta * (-1) * np.random.randint(0, 1, [N, D])
    beta[np.random.rand(N, D) < 0.5] = 1
    beta[np.tile(np.random.rand(N, 1) > proC, D)] = 1
    ...
    mu    = np.random.rand(N, D)
    mu_tent = (mu <= 0.5)
    temp  = np.bitwise_and(Site, mu_tent)
    Offspring = np.maximum(np.minimum(Upper, Offspring), Lower)
...
def update_animals_10(particles, fit, gbest, fitness_g, shrink, L, hunger, total_hunger, eps, in_, vmax, vmin):
    # for i in range(particles):
    R = 2 * shrink * np.random.rand() - shrink
    ...
    for i in range(particles):
        ...
        r2 = np.random.rand()
        if r1 < L:
            in_[i] = in_[i] * (1 + np.random.normal(0, 10))
        else:
            if r2 > E:
                in_[i] = W1 * gbest[i] + R * W2 * abs(gbest[i] - in_[i])
            else:
...
def update_animals_01(particles, fit, gbest, fitness_g, shrink, L, hunger, total_hunger, eps, in_, vmax, vmin):
    # for i in range(particles):
    R = 2 * shrink * np.random.rand() - shrink
    ...
    for i in range(particles):
        ...
        r2 = np.random.rand()
        if r1 < L:
            in_[i] = in_[i] * (1 + np.random.normal(0, 0.1))
        else:
            if r2 > E:
                in_[i] = W1 * gbest[i] + R * W2 * abs(gbest[i] - in_[i])
            else:
...
def update_animals_001(particles, fit, gbest, fitness_g, shrink, L, hunger, total_hunger, eps, in_, vmax, vmin):
    # for i in range(particles):
    R = 2 * shrink * np.random.rand() - shrink
    ...
    for i in range(particles):
        ...
        r2 = np.random.rand()
        if r1 < L:
            in_[i] = in_[i] * (1 + np.random.normal(0, 0.01))
        else:
            if r2 > E:
                in_[i] = W1 * gbest[i] + R * W2 * abs(gbest[i] - in_[i])
            else:
...
def update_animals_0001(particles, fit, gbest, fitness_g, shrink, L, hunger, total_hunger, eps, in_, vmax, vmin):
    # for i in range(particles):
    R = 2 * shrink * np.random.rand() - shrink
    ...
    for i in range(particles):
        ...
        r2 = np.random.rand()
        if r1 < L:
            in_[i] = in_[i] * (1 + np.random.normal(0, 0.001))
        else:
            if r2 > E:
                in_[i] = W1 * gbest[i] + R * W2 * abs(gbest[i] - in_[i])
            else:
...
def update_animals_tvl(particles, fit, gbest, fitness_g, shrink, L, hunger, total_hunger, eps, in_, vmax, vmin,cur_iter):
    # for i in range(particles):
    R = 2 * shrink * np.random.rand() - shrink
...
def update_animals_00001(particles, fit, gbest, fitness_g, shrink, L, hunger, total_hunger, eps, in_, vmax, vmin):
    # for i in range(particles):
    R = 2 * shrink * np.random.rand() - shrink
    ...
    for i in range(particles):
        ...
        r2 = np.random.rand()
        if r1 < L:
            in_[i] = in_[i] * (1 + np.random.normal(0, 0.0001))
        else:
            if r2 > E:
                in_[i] = W1 * gbest[i] + R * W2 * abs(gbest[i] - in_[i])
            else:
...
def update_animals_with_archive(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    ...
    for i in range(N):
        ...
        for j in range(D):
            if random.random() > cr_ or j == j_rand:  # uncooperative animals
                d_val = in_[i, j] * (1 + np.random.normal(0, 1))
            else:
                if cur_iter < 400:  # exploration
                    d_val = W1 * archive_in[randNum_, j] + R * W2 * abs(pbest[i, j] - in_[i, j])
                else:  # exploitation
...
def update_animals_with_archive_10(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
    ...
    for i in range(N):
        ...
        for j in range(D):
            if random.random() > cr_ or j == j_rand:  # uncooperative animals
                d_val = in_[i, j] * (1 + np.random.normal(0, 10))
            else:
                if cur_iter < 400:  # exploration
                    d_val = W1 * archive_in[randNum_, j] + R * W2 * abs(pbest[i, j] - in_[i, j])
                else:  # exploitation
...
def update_animals_with_archive_01(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
    ...
    for i in range(N):
        ...
        for j in range(D):
            if random.random() > cr_ or j == j_rand:  # uncooperative animals
                d_val = in_[i, j] * (1 + np.random.normal(0, 0.1))
            else:
                if cur_iter < 400:  # exploration
                    d_val = W1 * archive_in[randNum_, j] + R * W2 * abs(pbest[i, j] - in_[i, j])
                else:  # exploitation
...
def update_animals_with_archive_001(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
    ...
    for i in range(N):
        ...
        for j in range(D):
            if random.random() > cr_ or j == j_rand:  # uncooperative animals
                d_val = in_[i, j] * (1 + np.random.normal(0, 0.01))
            else:
                if cur_iter < 400:  # exploration
                    d_val = W1 * archive_in[randNum_, j] + R * W2 * abs(pbest[i, j] - in_[i, j])
                else:  # exploitation
...
def update_animals_with_archive_0001(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
    ...
    for i in range(N):
        ...
        for j in range(D):
            if random.random() > cr_ or j == j_rand:  # uncooperative animals
                d_val = in_[i, j] * (1 + np.random.normal(0, 0.001))
            else:
                if cur_iter < 400:  # exploration
                    d_val = W1 * archive_in[randNum_, j] + R * W2 * abs(pbest[i, j] - in_[i, j])
                else:  # exploitation
...
def update_animals_with_archive_l(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
    ...
    for i in range(N):
        ...
        randNum_ = np.random.randint(0, indexes_)
        for j in range(D):
            if random.random() < L:  # uncooperative animals
                d_val = in_[i, j] * (1 + np.random.normal(0, 0.001))
            else:
                if cur_iter < 400:  # exploration
                    d_val = W1 * archive_in[randNum_, j] + R * W2 * abs(pbest[i, j] - in_[i, j])
    ...
    return in_, cr
def update_animals_with_archive_l006(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
    ...
    return in_, cr
def update_animals_with_archive_l007(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
    ...
    return in_, cr
def update_animals_with_archive_l008(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
    ...
    return in_, cr
def update_animals_with_archive_l009(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
...
def update_animals_with_archive_MS(gbest, shrink, L, hunger, total_hunger, eps, in_, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    R = 2 * shrink * np.random.rand() - shrink
    cr_ = 1 - (0.55 + 1 / np.pi * np.arctan((1 - cur_iter / 1000 - 0.8) / 0.3))
    for i in range(N):
        ...
        for j in range(D):
            if random.random() > cr_ or j == j_rand:  # uncooperative animals
                ...
                if np.random.rand() < 0.7:  # exploration
                    d_val = W1 * archive_in[randNum_, j] + R * W2 * abs(pbest[i, j] - in_[i, j])
                else:  # exploitation
                    d_val = W1 * gbest[i, j] + R * W2 * abs(gbest[i, j] - in_[i, j])
...
def update_animals_with_archive_00001(gbest, shrink, L, hunger, total_hunger, eps, in_, T2, cr, fit, fit_g, cur_iter, archive_in, pbest):
    N, D = in_.shape
    for i in range(N):
        if np.random.uniform(0, 1) < T2:
            cr[i] = np.random.uniform(0, 1)
...
###########################################  SMA  ###########################################
def update_SMA(particles, fit, in_g, fitness_g, fitness_w, z, eps, in_, SMA_a, SMA_b, weight):
    old_weight = weight
    for i in range(particles):
        # sig = fitness_g[i, 1] - fitness_w + eps
        sig = 0.5 * (fitness_g[i, 1] - np.max(fit[:, 1])) + 0.5 * (fitness_g[i, 0] - np.max(fit[:, 0])) + eps
        # if i <= int(N / 2):
        if random.uniform(0, 1) < int(particles / 2):
            # weight[i] = 1 + random.uniform(0, 1) * np.log10((fitness_g[i, 1] - fit[i, 1]) / sig + 1)
            weight[i] = 1 + random.uniform(0, 1) * np.log10(
                (0.5 * (np.min(fitness_g[:, 1]) - fit[i, 1]) + 0.5 * (np.min(fitness_g[:, 0]) - fit[i, 0])) / sig + 1)
        else:
            # weight[i] = 1 - random.uniform(0, 1) * np.log10((fitness_g[i, 1] - fit[i, 1]) / sig + 1)
            weight[i] = 1 - random.uniform(0, 1) * np.log10(
                (0.5 * (np.min(fitness_g[:, 1]) - fit[i, 1]) + 0.5 * (np.min(fitness_g[:, 0]) - fit[i, 0])) / sig + 1)
        if weight[i] == np.NaN or np.NAN:
            weight[i] = old_weight[i]

    
    for i in range(particles):
        if np.random.uniform() < z:  # Eq.(2.7)
            # X[i, d] = random.uniform(lb[d], ub[d])
            in_[i] = in_[i] * (1 + np.random.normal(0, 1))
        else:
            # p = np.tanh(abs(fit[i] - fitness_g[i]).any())
            p = np.tanh(0.5 * abs(fit[i, 0] - fitness_g[i, 0]) + 0.5 * abs(fit[i, 1] - fitness_g[i, 1]))
            vb = random.uniform(-SMA_a, SMA_a)  # Eq.(2.3)
            vc = random.uniform(-SMA_b, SMA_b)

            # two positions randomly selected from population, apply for the whole problem size instead of 1 variable
            
            # id_a, id_b = random.choice(list(set(range(particles)) - {i}), 2, replace=False)
            ids_except_current = [_ for _ in range(particles) if _ != i]
            id_a, id_b = random.sample(ids_except_current, 2)

            if np.random.uniform() < p:  # Eq.(2.1)
                in_[i] = in_g[i] + vb * (weight[i] * in_[id_a] - in_[id_b])
            else:
                in_[i] = vc * in_[i]
...
def update_WOA(particles, in_g, in_, a, a2):
    for i in range(particles):
        ...
        if p < 0.5:
            if abs(A) >= 1:
                ...
                X_rand = in_[rand_leader_index, :]
                D_X_rand = abs(C * X_rand - in_[i])
                in_[i] = X_rand - A * D_X_rand

            elif abs(A) < 1:
                D_Leader = abs(C * in_g[i] - in_[i])
                in_[i] = in_g[i] - A * D_Leader

        elif p >= 0.5:

            distance2Leader = abs(in_g[i] - in_[i])
            # Eq. (2.5)
            in_[i] = (
...
def update_HS(particles, in_g, in_, cr, pa_r, dyn_fw, fw_damp):
    delta_ = np.zeros((deed.NO_OF_HOURS, deed.NO_OF_GENERATORS))
    delta = np.zeros((deed.NO_OF_HOURS, deed.NO_OF_GENERATORS))
    for h in range(deed.NO_OF_HOURS):
        for g in range(deed.NO_OF_GENERATORS):
            delta_[h, g] = np.random.normal(deed.GENERATORS_MIN_POWER[g], deed.GENERATORS_MAX_POWER[g])
    # delta_ = delta_.reshape(deed.NO_OF_HOURS * deed.NO_OF_GENERATORS)
    for h in range(deed.NO_OF_HOURS):
        delta[h] = dyn_fw * delta_[h]
    delta = delta.reshape(deed.NO_OF_HOURS * deed.NO_OF_GENERATORS)
    for i in range(particles):
        if np.random.rand() < cr:
            in_[i] = in_g[i]
        in_new = in_[i] + delta
        if np.random.rand() < pa_r:
            in_[i] = in_new

        in_[i] = ensure_min_max_constraints(in_[i])
        in_[i] = adjust_slack_power(in_[i])
    dyn_fw = dyn_fw * fw_damp
    return in_, dyn_fw
