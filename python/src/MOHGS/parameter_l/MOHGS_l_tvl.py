...
class MoHGS_l_tvl:
    ...
    def update_(self):
        ...
        self.in_ = update.update_animals_tvl(self.particals, self.fitness_, self.in_g, self.fitness_g, self.shrink, self.L,
                                         self.hunger_list, self.total_hunger, self.eps, self.in_, self.max_, self.min_,self.cur_iter)
    ...
    def done(self,cycle_):
        ...
        for i in range(cycle_):
            ...
            print('Iteration ',i,' completed, MOHGS_l=tvl best_emission, MOHGS best_cost: ', np.min(self.fitness_g[:, 1]), np.min(self.fitness_g[:, 0]))
