...
class MOHjDE:
    def __init__(self,particals,max_,min_,thresh,mesh_div=100, LH=10000, L=0.08):
    ...
    def update_(self):
        ...
        self.in_, self.f, self.cr = update.update_HjDE(self.in_, self.f, self.cr, self.T1, self.T2, self.Fl, self.Fu, self.min_, self.max_, self.in_g, self.shrink, self.L,
                                         self.hunger_list, self.total_hunger, self.eps)
    ...
    def done(self,cycle_):
        ...
        for i in range(cycle_):
            ...
            # self.L = 0.1 * np.e ** ((-1) * ((i / cycle_) ** 1))
            self.update_()

            
            print('Iteration ',i,' completed, MOHjDE best_emission, MOHjDE best_cost: ', np.min(self.fitness_g[:, 1]), np.min(self.fitness_g[:, 0]))
