...
import time
class LMOHADE_1000:
    def __init__(self,particals,max_,min_,thresh,mesh_div=10, LH=1000, L=0.08):
    ...
    def update_(self):
        ...
        self.in_, self.cr = update.update_animals_with_archive_0001(self.in_g, self.shrink, self.L, self.hunger_list, self.total_hunger, self.eps, self.in_, self.T2, self.cr, self.fitness_, self.fitness_g, self.cur_iter, self.archive_in, self.in_p)
    ...
    def done(self,cycle_):
        ...
        for i in range(cycle_):
            ...
            self.update_()
            self.cur_iter = i
            print('Iteration ', i, ' completed, L-MOHADE_th_1000_new best_emission, L-MOHADE best_cost: ',
