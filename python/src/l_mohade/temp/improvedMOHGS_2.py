...
class MOIHGS:
    ...
    def update_(self):
        ...
        self.in_, self.cr = update.update_animals_improvedHGS_test2(self.in_g, self.shrink, self.L, self.hunger_list, self.total_hunger, self.eps, self.in_, self.T2, self.cr, self.in_p, self.cur_iter, self.max_, self.min_, self.delta_list)
    ...
    def done(self,cycle_):
        ...
        for i in range(cycle_):
            ...
            print('Iteration ',i,' completed111, improvedHGS_test2 best_emission, improvedHGS best_cost: ', np.min(self.fitness_g[:, 1]), np.min(self.fitness_g[:, 0]))
