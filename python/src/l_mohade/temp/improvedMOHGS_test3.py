...
class MOIHGS:
    def __init__(self,particals,max_,min_,thresh,mesh_div=10, LH=10000, L=0.08):
        ...
        self.fitness_before = 0
        self.cr_m = 0.5
        self.cr = np.clip(np.random.normal(self.cr_m, 0.1, particals), 0, 1)
    ...
    def initialize(self):
        ...
        self.archive_in, self.archive_fitness, self.archive_cr = init.init_archive_and_cr(self.in_, self.fitness_, self.cr)
    ...
    def update_(self):
        ...
        self.in_, self.cr = update.update_animals_improvedHGS_test3(self.in_g, self.shrink, self.L, self.hunger_list, self.total_hunger, self.eps, self.in_, self.T2, self.cr, self.fitness_, self.fitness_g, self.cur_iter, self.max_iter, self.in_p)
    ...
    def done(self,cycle_):
        ...
        for i in range(cycle_):
            ...
            print('Iteration ',i,' completed111, improvedHGS_3333 best_emission, improvedHGS_3 best_cost: ', np.min(self.fitness_g[:, 1]), np.min(self.fitness_g[:, 0]))
