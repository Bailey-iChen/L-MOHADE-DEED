...
class MOIHGS:
    ...
    def update_(self):
        ...
        self.in_, self.cr = update.update_animals_improvedHGS_test1(self.in_g, self.shrink, self.L, self.hunger_list, self.total_hunger, self.eps, self.in_, self.T2, self.cr, self.fitness_, self.fitness_g, self.cur_iter)
        self.evaluation_fitness()
    ...
    def done(self,cycle_):
        ...
        for i in range(cycle_):
            ...
            print('Iteration ',i,' completed1112, improvedHGS best_emission, improvedHGS best_cost: ', np.min(self.fitness_g[:, 1]), np.min(self.fitness_g[:, 0]))
