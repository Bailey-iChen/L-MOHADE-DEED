...
import time
class LMOHADE_l_007:
    def __init__(self,particals,max_,min_,thresh,mesh_div=10, LH=10000, L=0.07):
    ...
    def update_(self):
        ...
        self.in_, self.cr = update.update_animals_with_archive_l007(self.in_g, self.shrink, self.L, self.hunger_list, self.total_hunger, self.eps, self.in_, self.T2, self.cr, self.fitness_, self.fitness_g, self.cur_iter, self.archive_in, self.in_p)
    ...
    def done(self,cycle_):
        ...
        for i in range(cycle_):
            ...
            self.update_()
            self.cur_iter = i
            print('Iteration ', i, ' completed, L-MOHADE_l_007_new best_emission, L-MOHADE best_cost: ',
