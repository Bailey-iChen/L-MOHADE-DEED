...
class LMOHADE:
    ...
    def update_(self):
        ...
        # The shared hunger helper reads one worst value per objective.
        self.fitness_w = np.max(self.fitness_, axis=0)
        ...
        self.in_, self.cr = update.update_animals_with_archive(self.in_g, self.shrink, self.L, self.hunger_list, self.total_hunger, self.eps, self.in_, self.T2, self.cr, self.fitness_, self.fitness_g, self.cur_iter, self.archive_in, self.in_p)
    ...
    def done(self,cycle_):
        ...
        for i in range(cycle_):
            ...
            self.update_()
            self.cur_iter = i
            print('Iteration ', i, ' completed, L-MOHADE best_emission, L-MOHADE best_cost: ',
